...
class FourKImageProcessor {
    ...
    createBadgeSvg(badgeText) {
        const text = badgeText || '4K Wallpapers';
        ...
        <svg width="${badgeWidth}" height="${badgeHeight}" viewBox="0 0 ${badgeWidth} ${badgeHeight}" xmlns="http://www.w3.org/2000/svg">
            <defs>
                <linearGradient id="fourKGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <rect x="2" y="2" width="${badgeWidth - 4}" height="${badgeHeight - 4}" rx="22"
                  fill="url(#fourKGrad)" stroke="rgba(255, 255, 255, 0.28)" stroke-width="1.2" filter="url(#dropShadow)" />

            <!-- 4K Icon Circle -->
            <circle cx="24" cy="24" r="14" fill="#22c55e" />
            <text x="24" y="29" text-anchor="middle" fill="#FFFFFF" font-family="Segoe UI, -apple-system, BlinkMacSystemFont, Roboto, sans-serif"
                  font-size="13" font-weight="700">4K</text>
    ...
    async processForDesktop(inputPath, badgeText = '') {
        ...
        if (isPortraitOrNarrow) {
            ...
            console.log(`[ImageProcessor] Generating widescreen wallpaper with blurred side fills (${screenWidth}x${screenHeight})...`);
...
module.exports = new FourKImageProcessor();
